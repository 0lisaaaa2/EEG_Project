...
def main_pipeline():
    ...
    grand_avg_erp_front_sym, grand_avg_erp_front_asym, grand_avg_erp_persp_sym, grand_avg_erp_persp_asym = s_11_grand.compute_grand_erp(
        grand_erps)
    ...
    # Average amplitudes
    grand_avg_cost_amp, grand_avg_spn_front_amp, grand_avg_spn_persp_amp = s_11_grand.compute_grand_amps(
        grand_cost_amps, grand_spn_amps)

    print(
        f"Averaged amplitudes across all subjects: \n Frontoparallel: {grand_avg_spn_front_amp} \n Perspective: {grand_avg_spn_persp_amp} \n Perspective Cost: {grand_avg_cost_amp}")

    # Plotting the results :)

    # ERP vs SPN — Frontoparallel
    s_11_grand.plot_spn_vs_erps(erp_sym=grand_avg_erp_front_sym, erp_asym=grand_avg_erp_front_asym,
                                spn=grand_avg_spn_front)

    # ERP vs SPN — Perspective
    s_11_grand.plot_spn_vs_erps(erp_sym=grand_avg_erp_persp_sym, erp_asym=grand_avg_erp_persp_asym,
                                spn=grand_avg_spn_persp)

    # SPNs vs Perspective Cost
    s_11_grand.plot_spns_vs_pc(spn_front=grand_avg_spn_front, spn_persp=grand_avg_spn_persp,
                               perspective_cost=grand_avg_cost)

    s_11_grand.plot_topography(spn_front=grand_avg_spn_front, spn_persp=grand_avg_spn_persp,
                               perspective_cost=grand_avg_cost)
    ...
    # Statistics
    s_13_stat.run_statistics(spn_front_values=grand_spn_amps['regfront'], perspective_cost_values=grand_cost_amps,
                             alpha=0.02)  # alpha = 0.011?
    s_13_stat.plot_spn_amplitude(grand_avg_cost_amp, grand_avg_spn_front_amp, grand_avg_spn_persp_amp, grand_cost_amps,
                                 grand_spn_amps['regfront'], grand_spn_amps['regperp'], alpha=0.02)
